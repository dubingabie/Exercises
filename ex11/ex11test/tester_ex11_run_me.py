...
from ex11 import Diagnoser, Record, build_tree, optimal_tree, parse_data
...
def analyze_raw_file(lines, filename):
    ...
    for i in tqdm(range(loop)):
        ...
        if match is not None:
            if found_main:
                warnings.warn(
                    CF.RED + "ERROR - You have to main blocks in your code!")
            found_main = True
        ...
        if not regex.match(DEF_REGEX_pep8, line):
            warning_lst.append(
                CF.YELLOW + "pep8 Warning: method name is probably not pep8 in line " + str(
                    i) + ' : ' + line)
    time.sleep(0.2)
    func_lengths.append(def_len_counter)
...
def test_raw_code(code, *filenames):
    ...
    for file_index in range(len(filenames)):
        ...
        consts, funcs, func_lens, imports, hasMain = analyze_raw_file(lines,
                                                                      filenames[
                                                                          file_index])
        def_count += len(funcs)
        const_count += consts
        ...
        if 'tester_helper_dont_touch' in imports or 'tester_ex11_run_me' in imports:
            print(
                CF.RED + "You may not import files of this tester (-it will not run on the university's computers...")
            return False
...
def test_main():
    print(
        CF.LIGHTBLUE_EX + 'Before we start running the rest of the tests, we would like to give you a debugging tip:')
    print(
        CF.LIGHTBLUE_EX + 'If you need to create a binary tree to test a method you can you the function "tree_builder" from the file "tester_helper_dont_touch".')
    print(
        CF.LIGHTBLUE_EX + "The function takes a str and turns it into a tree. each word is a node and a . represents that the node is a leaf")
    print(
        CF.LIGHTBLUE_EX + "After writing a non-leaf node you write the positive_child tree and then the negative_child tree.")
    print(
        CF.LIGHTBLUE_EX + "*You can also you the function 'tree_to_str' from the same file to print a tree nicely")
    ...
    time.sleep(1)
    tests = [init_test, test_diagnose, test_success_rate, test_all_illnesses,
             test_paths_to_illness,
             test_build_tree, test_optimal_tree, test_optimal_tree_exeptions,
             test_minimize_1,
             test_minimize_2]
    passed = 0
    for i, test in enumerate(tests):
        ...
        if bul: print(
            CF.LIGHTMAGENTA_EX + f'You passed the {ordinal(i)} test!' + f' (tests "{test.__name__[5:]}") ' if i > 0 else '')
    if passed < len(tests):
        print(CF.BLUE +
              f"\n\nYou passed {passed} tests out of {len(tests)}. A little work and you'll do much better!")
        return False
    print(CF.GREEN + (
        f"We'd like to tell you congradulations for passing all {passed} tests,"
...
def init_test():
    ...
    try:
        try:
            ...
            another_node = Node(data='data', positive_child=nodeA,
                                negative_child=nodeB)
        except:
            print(
                CF.LIGHTRED_EX + 'Error - while constructing a Node object. The error details are printed below:')
            raise
        try:
        ...
        try:
            ...
            print(
                CF.LIGHTRED_EX + 'Error - while constructing a Record object. The error details are printed below:')
            raise
    except:
        time.sleep(0.01)
        traceback.print_exc()
...
def test_diagnose():
    if not Method("diagnoser.diagnose", diagnoser1.diagnose, "cold",
                  ["cough"], diagnoser1):
        return False
    if not Method("diagnoser.diagnose", diagnoser1.diagnose, "healthy", [],
                  diagnoser1):
        return False
    if not Method("diagnoser.diagnose", diagnoser2.diagnose, "influenza",
                  ["cough", "fever"], diagnoser2):
        return False
    if not Method("diagnoser.diagnose", diagnoser2.diagnose, "healthy", [],
                  diagnoser2):
        return False
    if not Method("diagnoser.diagnose", diagnoser2.diagnose, "cold",
                  ["cough"], diagnoser2):
        return False
    if not Method("diagnoser.diagnose", diagnoser2.diagnose, "influenza",
                  ["cough", "fever"], diagnoser2):
        return False
    if not Method("diagnoser.diagnose", diagnoser2.diagnose,
                  "hard influenza", ["cough", "fever", "headache"],
                  diagnoser2):
        return False
    if not Method("diagnoser.diagnose", diagnoser3.diagnose,
                  "hard influenza", [], diagnoser3):
        return False
    if not Method("diagnoser.diagnose", diagnoser3.diagnose,
                  "hard influenza", ["cough", "fever", "headache"],
                  diagnoser3):
        return False
    if not Method("diagnoser.diagnose", diagnoser3.diagnose, "cold",
                  ["cough"], diagnoser3):
        return False
    if not Method("diagnoser.diagnose", diagnoser3.diagnose, "influenza",
                  ["cough", "fever"], diagnoser3):
        return False
    if not Method("diagnoser.diagnose", diagnoser4.diagnose,
                  "hard influenza", ["cough", "fever", "headache"],
                  diagnoser4):
        return False
    if not Method("diagnoser.diagnose", diagnoser4.diagnose, "influenza",
                  [], diagnoser4):
        return False
    if not Method("diagnoser.diagnose", diagnoser4.diagnose, "cold",
                  ["cough"], diagnoser4):
        return False
    if not Method("diagnoser.diagnose", diagnoser4.diagnose, "influenza",
                  ["cough", "fever"], diagnoser4):
        return False
    if not Method("diagnoser.diagnose", diagnoser5.diagnose, "influenza",
                  [], diagnoser5):
        return False
    if not Method("diagnoser.diagnose", diagnoser5.diagnose, "cold",
                  ["headache"], diagnoser5):
        return False
    if not Method("diagnoser.diagnose", diagnoser5.diagnose, "cold",
                  ["cough"], diagnoser5):
...
def test_success_rate():
    ...
    if not Method("diagnoser.calculate_success_rate",
                  diagnoser1.calculate_success_rate, 0.6666666666666666,
                  records, diagnoser1):
        return False
    records = [Record("influenza", ["cough", "fever"]), Record("healthy", []),
               Record('hard influenza', ["cough", "fever", "headache"])]
    if not Method("diagnoser.calculate_success_rate",
                  diagnoser2.calculate_success_rate, 1.0, records,
                  diagnoser2):
        return False
    records = [Record("influenza", ["cough", "fever"]),
               Record("indigestion", ["stomachache"])]
    if not Method("diagnoser.calculate_success_rate",
                  diagnoser2.calculate_success_rate, 0.5, records,
                  diagnoser2):
        return False
    if not Method("diagnoser.calculate_success_rate",
                  diagnoser2.calculate_success_rate, 0.0,
                  [Record("a", ["b"])], diagnoser2):
        return False
    records = []  # test an error is thrown as requested
    if not Method_err("diagnoser.calculate_success_rate",
                      diagnoser1.calculate_success_rate, ValueError,
                      records,
                      diagnoser1):
        return False
...
def test_all_illnesses():
    if not Method("diagnoser.all_illnesses", diagnoser5.all_illnesses,
                  ['cold', 'influenza'],
                  tuple(), diagnoser5, check_sorted=True):
        return False
    if not Method("diagnoser.all_illnesses", diagnoser4.all_illnesses,
                  ["hard influenza", 'cold', 'influenza'],
                  tuple(), diagnoser4, check_sorted=True):
        return False
    if not Method("diagnoser.all_illnesses", diagnoser3.all_illnesses,
                  ["hard influenza", 'cold', 'influenza'],
                  tuple(), diagnoser3, check_sorted=True):
        return False
    if not Method("diagnoser.all_illnesses", diagnoser2.all_illnesses,
                  ['healthy', 'cold', 'influenza', 'hard influenza'],
                  tuple(), diagnoser2, check_sorted=True):
        return False
    if not Method("diagnoser.all_illnesses", diagnoser1.all_illnesses,
                  ['healthy', 'cold', 'influenza'],
                  tuple(), diagnoser1, check_sorted=True):
        return False
    actual = diagnoser3.all_illnesses()
    if not actual[0] == 'hard influenza':
        print_fail('diagnoser.all_illnesses', tuple(),
                   ["hard influenza", 'cold', 'influenza'], actual, diagnoser3,
...
def test_paths_to_illness():
    ...
    for test in tests:
        ...
        if not Method("diagnoser.paths_to_illness", obj.paths_to_illness,
                      expected, input, obj,
                      check_sorted=True):
            return False
    return True
...
def test_build_tree():
    ...
    for i in range(len(args_lst)):
        ...
        if not Method('build_tree', build_tree, expected, args,
                      builtTree=True):
            return False
    args = args_lst[0]
    for val in [None, 3]:
        for i in range(2):
            new_args = tuple(
                [arg + [val] if j == i else arg for j, arg in enumerate(args)])
            if not Method_err('build_tree', build_tree, TypeError,
                              new_args, obj=None):
                return False
    void_recs = [Record(None, [])]
    vals = ['a'], ['a', 'b', 'c']
    expecs = ['a None . None .',
              'a' + ' b c None . None . c None . None .' * 2]
...
def test_optimal_tree():
    data = lambda i: f"tests_data/test_optimal_tree{i}.txt"
    args_lst = [
    ...
    for i in range(len(args_lst)):
        ...
        test_method = Method if len(answers) == 1 else Method_options
        if len(answers) == 1: answers = answers[0]
        if not test_method('optimal_tree', optimal_tree, answers, args,
                           builtTree=True):
            return False
    return True
...
def test_optimal_tree_exeptions():
    ...
    for depth in [-1, -12, len(args[1]) + 1]:
        ...
        if not Method_err('optimal_tree', optimal_tree, ValueError,
                          new_args):
            return False
    for val in [None, 0.4]:
        for i in range(2):
            new_args = tuple(
                [arg + [val] if j == i else arg for j, arg in enumerate(args)])
            if not Method_err('optimal_tree', optimal_tree, TypeError,
                              new_args):
                return False
    return True
...
def test_minimize_1():
    ...
    expected_1 = Diagnoser(
        tree_builder('cough fever covid-19 . cold . sweat None . healthy .'))
    expected_2 = Diagnoser(
        tree_builder('cough fever covid-19 . cold . healthy .'))
    if not Method_mutableObj('diagnoser.minimize', diagnoser.minimize,
                             expected_1, tuple(), obj=diagnoser):
        return False
    if not Method_mutableObj('diagnoser.minimize', diagnoser.minimize,
                             expected_2, True, obj=diagnoser):
        return False
...
def test_minimize_2():
    ...
    for i in tqdm(range(len(answers))):
        diagnoser = Diagnoser(tree_builder(trees[i][0])) if type(
            trees[i][0]) is str else trees[i][0]
        expected = Diagnoser(tree_builder(answers[i]))
        if not Method_mutableObj('diagnoser.minimize', diagnoser.minimize,
                                 expected, trees[i][1], obj=diagnoser):
            return False
    return True
...
def generate_vec(dim):
    ...
    if not (len(symptoms) >= depth >= 0):
        raise ValueError(
            f'the depth argument must be between len(symptoms) and 0 - given {depth}')
    if sorted(symptoms) != sorted(list(set(symptoms))):
        raise ValueError(
            'the symptoms list may not include the same symptom twice!')
    for record in records:
        if type(record) is not Record:
            raise ValueError(
                'all elements in the records list must be of type Record!')
    for symp in symptoms:
        if type(symp) is not str:
            raise ValueError(
                'all elements in the symptoms list must be of type str!')
    if len(symptoms) == 0:
...
def repr_obj(obj, title=True):
    ...
    if type(obj) == list: return '[' + ', '.join(
        [repr_obj(e) for e in obj]) + ']'
    return str(obj)
...
def print_fail(method_name, args, expected, actual, object, issue=None,
               exp_note=''):
...
def Method_err(func_name, check_func, err_type, args, obj=None):
    if type(args) != tuple:
        args = (args,)
    ...
    try:
        ...
        if type(e) is err_type:
            return True
        print_fail(func_name, args, f"Error of type {err_type}",
                   f"Error of type {type(e)}", obj)
        time.sleep(0.01)
        print(
            CF.LIGHTCYAN_EX + 'The Error details are specified below for you comfort:')
        time.sleep(0.01)
        traceback.print_exc()
        return False
    print_prob(func_name, args, f"Error of type {err_type}", f"Returned {val}",
               obj,
               problem="Didn't raise error when should have.")


def Method_options(func_name, check_func, expected_options, args, obj=None,
                   check_sorted=False,
    ...
    Method_predicate(func_name, check_func, predicate, expected_msg, args,
                     obj, mutableArgCheck, mutableObjCheck,
                     exp_note=exp_note)


def Method(func_name, check_func, expected, args, obj=None,
           check_sorted=False, mutableArgCheck=True,
    ...
    exp_note = TREE_EXP_NOTE if builtTree else ''
    return Method_predicate(func_name, check_func, predicate, expected_msg,
                            args, obj, mutableArgCheck,
                            mutableObjCheck, exp_note=exp_note)


def Method_arg(func_name, check_func, expected, args, arg_to_check: int,
               obj, check_sorted=False,
               mutableArgCheck=True, mutableObjCheck=True):
    expected_msg = expected

    def predicate(val, actual_args, original_args, obj):
        if check_sorted: return sorted(actual_args[arg_to_check]) == sorted(
            expected)
...
def Method_mutableObj(func_name, check_func, expected, args, obj,
                      mutableArgCheck=True):
    expected_msg = expected
...
def Method_predicate(func_name, check_func, predicate, expected_msg, args,
                     obj=None, mutableArgCheck=True,
...
def main():
    try:
        colorInit()
        with CWDLoader():
            should_test_zip = input(
                'type a number if you wish this program to read from a zip file:')
            ...
            if should_test_zip:
                ...
                if not ENABLE_ZIP:
                    ...
                    print(
                        'Please run this tester on a python file, Thank you :) \n')
                    sys.exit(0)
        ...
        print(CF.MAGENTA + 'could not find/load required files from folder:',
              d)
        raise ose
    except Exception:
        print(
            CF.MAGENTA + "the tester has thrown an error. please report to yair =) ")
        raise
    finally:
        time.sleep(0.1)
