...
class Diagnoser:
    ...
    def all_illnesses(self):
        ...
        illnesses_dict = dict()
        pointer = self.root
        self.__all_illnesses_core(pointer, illnesses_dict)
        return list(map(lambda x: x[0], sorted(illnesses_dict.items(), key=lambda item: item[1], reverse=True)))

    def __all_illnesses_core(self, pointer, illnesses_dict):
        """
        a recursive core function the all_illneses function
        :param pointer: a pointer the the Diagnoser root
        :param illnesses_dict: an list which all of the illnesses to the tree will be added to
        :return: None
        """
        if pointer is None:
            return
        if pointer.positive_child is None and pointer.negative_child is None:
            if pointer.data:
                if pointer.data not in illnesses_dict.keys():
                    illnesses_dict[pointer.data] = 0
                illnesses_dict[pointer.data] += 1
        self.__all_illnesses_core(pointer.positive_child, illnesses_dict)
        self.__all_illnesses_core(pointer.negative_child, illnesses_dict)
    ...
    def __minimize_core(self, root, remove_empty):
        ...
        if self.__is_equal(positive_diagnose, negative_diagnose):
            self.__replace_root(root, positive_diagnose)
        self.__replace_empty(remove_empty, root, positive_diagnose, negative_diagnose)
        return root

    def __replace_empty(self, remove_empty, root, positive_diagnose, negative_diagnose):
        """
        this is a helper functions that removes the empty cells according to the instructions given in the exercise
        if remove_empty is true in each recursive iteration
        :param remove_empty: a bool type that states if the empty cells should be removed
        :param root: the root of the given tree
        :param positive_diagnose: the recursive return value of the positive child
        :param negative_diagnose: the recursive return value of the negative child
        :return: None
        """
        if remove_empty and \
            ...
            if positive_diagnose.data is not None:
                ...
                self.__replace_root(root, negative_diagnose)

    def __replace_root(self, root, new_root):
        """
        this is a helper function to minimize that replaces the root of the given tree
        with the root given
        :param root: a Node type that contains the root of the tree that is to be replaced
        :param new_root: a Node type that contains the root that will replace the original root
        :return: None
        """
        root.data = new_root.data
        root.positive_child = new_root.positive_child
        root.negative_child = new_root.negative_child

    def __is_equal(self, root1, root2):
        """
        a recursive function that checks if two trees are identical
        :param root1: a Node type that contains the root of the first tree
        :param root2: a Node type that contains the root fo the second tree
        :return:
        """
        if root1 is None and root2 is None:
            return True
        if root1.data == root2.data:
            return True and self.__is_equal(root1.positive_child, root2.positive_child) and \
                   self.__is_equal(root1.negative_child, root2.negative_child)
        return False
...
def build_tree(records, symptoms):
    ...
    if len(symptoms):
        ...
        root = Node(__most_common_illness(records))
...
def __most_common_illness(records):
    ...
    return sorted(illness_dict.items(), key=lambda item: item[1], reverse=True)[0][0] if len(illness_dict) else None
